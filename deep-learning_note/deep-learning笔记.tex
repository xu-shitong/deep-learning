...
\textbf{Transformer}
  ...
  \quad \quad 令词$i$有词向量$w_i$，则有query $W^Qw_i$，key $W^Kw_i$，value $W^Vw_i$
  ...
  \quad 使用head$H$组$W^Q, W^K, W^V \in \mathbb{R}^{D / H \times d_h}$。

  \quad 输出的H组$Z_i$连接后经过全连接层，张量形状$(T, d_h)$不变

  Masked multi-head self-attention：

  \quad 每一head权重部分$\frac{QK^T}{\sqrt{d_h}}$按元素相乘三角矩阵，三角矩阵中上三角为负常数，常取$-10^9$。

  \quad \quad 目的：softmax后使得$i$位置的词仅能和$[0, i]$范围内的词计算attention
  ...
  \quad 定义source mask：形状同encoder输入词序。所有'<pad>'词对应位置权重为极低值，保证softmax后其余词和此词attention值近似为0。
  ...
  \quad \quad 1. embedding，将词序转为词向量
  ...
  \quad 定义target mask：形状同decoder输入词序。
  
  \quad \quad 所有'<pad>'位置权重设为极小值

  \quad \quad **对每一T长度词序，分为$T \times T$个词序。第i号词序从i位置以后权重设为极低值，模拟逐词预测整个decoder输入词序**
  ...
  \quad \quad 2. masked multi-head self-attention：
  ...
  \quad \quad 1. embedding，position encoding。同encoder
...
\textbf{BERT}
  ...
  \quad \quad position embedding：在训练时更新\textbf{不同于encoder部分position embedding}
