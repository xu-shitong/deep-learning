...
\textbf{Transformer}
  ...
  \quad 使用head\ H组$W^Q, W^K, W^V \in \mathbb{R}^{D / H \times d_h}$。

  \quad 1. 输入词向量$w_q, w_k, w_v$分别经过3全连接层，得到的3向量各分为H段。即结果为H组$w_q, w_k, w_v$
  
  \quad 2. 1.中第i组$w_q, w_k, w_v$分别和第i组矩阵$W^Q, W^K, W^V$相称，得$w_qW^Q$，$w_kW^K$，$w_vW^K$。

  \quad 3. 得到的输出连接后经过另一全连接层，做最终multi head输出。张量形状$(T, d_h)$不变
  ...
  \quad mask中上三角为负常数，目的为：softmax后使得$i$位置的词仅能和$[0, i]$范围内的词计算attention
