...
\textbf{tanh函数}：
  ...
  平均值在0，允许输出为负值，帮助converge。计算开销较大
  
  多层tanh或sigmoid都将降低学习效率\\
\textbf{softmin}：$softmax(X) = \frac{exp(-x)}{\sum_{x' \in X} exp(-x') }$\\
\textbf{softmax函数}：$softmax(X) = \frac{exp(x)}{\sum_{x' \in X} exp(x') } $，将数值输出转化为概率值，1. 值为正\ 2. 值总和为1\\
\textbf{ReLU} 激活函数

  仅有一半neuron activate，前向计算更快速。dying relu problem：relu值为0导致权重不被更新\\
...
\textbf{Exponential LU}
  ...
  形状\ 从-1 按exp(x)增长到0，随后线性。\textbf{a从0增大时$x \leq 0$包含更大部分的exp图像，值$ > -\alpha$}
  
  \textbf{仅在$\alpha = 1$时continuously differentiable}\\
\textbf{CELU}

  \begin{equation*}
    ELU(x) = \begin{cases}
      \alpha (e^{\frac{x}{a}} - 1) & x < 0\\
      x & x \geq 0
    \end{cases}
  \end{equation*}
  ...
  \textbf{a从0增大时图像$x \leq 0$近似linear部分逐渐增大\ 增长变平缓，值$ > -\alpha$}

  \textbf{当a不为1时求导为continuous function 即continuously differentiable}\\
\textbf{SELU}

  $SELU(x) = scale * ELU, scale > 1$

  self normalizing：当每层激活函数使用internal normalizing函数时，最终的特征mean variance converge到一定值

  \quad 激活函数需要经过(0, 0)，有正负值。否则重复apply导致mean值为无限

  \quad 有区域输入绝对值较大时斜率较小。当较浅网络层variance较高时用于damp variance

  \quad 临近0位置斜率较高。增加较浅网络层variance

  \quad 为一continuous curve。保证有mean variance的fix point\\
\textbf{GELU}

  $GELU(x) = x \Phi(x)$

  $\Phi$为Gaussian的CDF。图像形状为在0附近有小幅降低后linear升高\\
\textbf{SoftPlus激活函数}：

  $SoftPlus(x) = \frac{1}{\beta} \log(1 + exp(\beta x))$

  形如exp，拐点连续可导。当$\beta$增大时，接近ReLU

  为保证Numerical stability，当$\beta x > $阈值时，改用线性激活函数\\
\textbf{LogSigmoid代价函数}：$\log(sigmoid(x))$形状约到x=2时线性上升到0，随后水平\\
\textbf{LogSoftmax代价函数}：$\log(softmax(x))$linear形状，不经过(0, 0)\\
\textbf{cross entropy}：代价函数

  定义：分部p\ 和\ 分部q\ 间的\ cross entropy $H(p, q) = -E_p(\log (q))$。为\ expected value of $log (q)$ with respect to distribution p
  
  \quad 针对单类别classification：$CELoss(x, target\_class) = -\log(\frac{\exp(x_{target\_class})}{\sum \exp(x)})$

  \quad \quad \textbf{即-LogSoftmax}\\
...
\textbf{Neg log likelihood loss} 代价函数
  ...
  \begin{equation*}
    NLLLoss(x, y) = \begin{cases}
      \sum_{n=1}^Nl_n & reduction=sum\\
      \sum_{n=1}^N\frac{l_n}{\sum_{n=1}^N w_n} & reduction=mean 
    \end{cases}
  \end{equation*}\\
\textbf{Margin Ranking loss}
  ...
  \quad $x_1, x_2$为两样本到类别centroid的距离\textbf{非两样本的embedding}，y为正代表$x_1$应当大于$x_2$ 常取$\pm 1$\\
\textbf{Triplet Margin Loss}

  $loss(x_a, x_p, x_n) = \max(0, m + |f(x_a) - f(x_b)| - |f(x_a) - f(x_n)|)$

  即\ 使正样本对$x_a, x_p$距离降为0 负样本对$x_a, x_n$距离高于threshould m\\
\textbf{Cosine Embedding loss}

  \begin{equation*}
    loss(x, y) = \begin{cases}
      1 - cosine(x_1, x_2) & y = 1\\
      \max(0, cosine(x_1, x_2) - m) & y = -1 
    \end{cases}
  \end{equation*}
  
  所有$x_i$特征为normalized。即\ 使正样本对$x_1, x_2$点乘接近1 负样本对$x_a, x_n$点乘小于threshould m\\
\textbf{指数加权移动平均}
...
\textbf{Bagging}
  ...
  \quad \quad 验证集out-off-bag oob 样本：当使用bagging选取时，平均只有$1-e^{-1}$样本被选择，余下样本称oob样本，为验证集
  ...
  \quad $k$feature importance特征重要性：对所有取$k$为判断条件的节点$N_i$，计算加权平均值$\sum_i(S_i$熵增百分比$)$\\
\textbf{boosting}

  将多个较弱模型合并为较强模型，每一模型使用之前模型预测效果较差样本作为训练集。降低偏差

  \quad AdaBoost：对上一预测机制遗漏的样本加更高权重，进行训练

  \quad gradient boosting：

  \quad \quad 第t时间合并的模型称$H_t$
  
  \quad \quad 第t模型$h_t$使用训练集$\{x_i, y_i - H_{t-1}(x_i)\}$，随后更新总模型$H_t = h_t + \eta h_t$

  \quad \quad 模型应使用regularization，否则较强模型导致拟合整个训练集

  决策树使用boosting：合并多模型即不同决策树输出取加权平均值

  \quad 训练sequential，耗时长，使用XGBoost lightGBM减少训练时间\\
\textbf{stacking}

  同一样本经过不同模型，输出连接后经过全连接层。降低方差

  multi stacking：
  
  \quad 多层stacking，每层输入为样本/前层连接结果

  \quad 易于overfitting，由于单层特征stacking已经为强模型

  \quad \quad 每层训练使用不同数据集。不同层通常使用相同模型，相同超参数，不共享权重

  \quad \quad \quad \textbf{训练第i层时使用第i训练集，输入为i训练集经过前层输出(不经过全连接层)，输出连接后经过全连接层进行预测}

  \quad \quad \quad \textbf{可对不同层输出预测取平均，降低偏差}

  \quad \quad 可使用k-fold创建不同层样本\\
\textbf{Neural Architecture Search搜索神经网络结构}

  使用RNN生成定义网络结构的文本。类似强化学习，环境即数据集，决策即网络结构

  One shot approach：训练极大模型，包含所有想要验证的模型

  \quad Differentiable Architecture Search：多层categorical search，每层包含多个不同超参数模型 + softmax

  \quad \quad softmax对每一模型输出加权平均，\textbf{输入的权重为可学习参数}

  \quad \quad 训练时输出为层中模型结果加权平均，预测时仅选择权重最大的唯一模型计算输出

  \quad \quad 由于权重可学习，称differentiable

  \quad DARTS

  Scaling CNN：efficientNet调整CNN参数时满足固定关系

  Hypernetworks/Neural Processes：预测其他网络超参数的网络\\
network with memory：kanerva model，非case by case模型
...
\textbf{AlexNet深度卷积神经网络}：
  ...
  除输出层和丢弃层，全部使用relu做激活函数，针对(224, 224, 3)形状图片
  ...
  \textbf{除最后一全连接层，所有参数都被分配到2GPU上，特征通道数被平分}
  
  \textbf{第三conv层和所有全连接层得到前层的所有输入进行计算}\\
\textbf{VGG使用重复元素网络}
...
\textbf{GoogLeNet含并行结构神经网络}
  ...
  \quad \textbf{使用inception块使得参数量比使用卷积层参数量更小}
  ...
  Inception-V2 (-BN)：使用batch normalization

  Inception-V3：修改inception块
  
  \quad 3.中的所有inception块(5, 5)卷积核换为2个串联(3, 3)卷积核
  
  \quad 4.中(3, 3)换为(7, 1)+(1, 7)卷积核，(5, 5)换为2组(7, 1)+(1, 7)卷积核
  
  \quad 5.中(3, 3)换为并行的(3, 1)和(1, 3)卷积核，(5, 5)换为(3, 3)+并行(3, 1)+(1, 3)卷积核
...
\noindent \textbf{批量归一化 batch normalization}
  ...
  \textbf{每一通道}有拉伸$\gamma$, 偏移$\beta$参数参与反向传播。初始化所有$\gamma = \vec{1}, \beta = \vec{0}$

  使用时无需和dropout合用，由于使用batch的mean/var导致每次训练有来自样本的noise，和dropout一样对特征加入noise。batch size 64-256间，保证计算的mean variance有noise
  ...
  \quad \textbf{$\mu' \sigma'^2$仅在预测时使用，训练时使用batch本身的mean var}

  \textbf{1. 对全连接层做批量归一}

  \quad 处于\ 输入的仿射变换\ 和\ 激活函数间，即\ 输出 = $\phi (BN(x))$ $BN$为批量归一计算

  \quad 1. 对于批量仿射 $x = Wu + b$，求标准化$\hat{x}_i = \frac{{x_i} - \mu_i}{\sqrt{\sigma_i^2 + \varepsilon }}$。$\mu $和 $\sigma^2$为$x$ mean var

  \quad \quad \textbf{即对不同样本同一特征向量元素求mean和var}

  \quad 2. $BN(x) = \gamma * \hat{x} + \beta $，$\gamma$拉伸 $\beta$偏移，和$x$形状相同。* + 为按元素加法 乘法
  ...
  \quad $\sigma^2$, $\mu$为此通道\ 所有样本内\ 的所有元素\ 的总体var，mean
...
\textbf{ResNet残差网络}
  ...
  \quad 解释1：由于网络输入常为(0, 1)区间的值，使用乘法导致前向传输值不断减小。残差网络将两层有助于维持输出值大小

  \quad \quad 存在前向传输路径跨过多层网络，有助于维持输出值大小

  \quad 解释2：
    
  \quad \quad generic function class：增加一网络层后不一定能够实现原先网络的预测\ 即增加网络层导致离正确网络结构更远。

  \quad \quad \quad 大多数没有残差连接网络为此类
  
  \quad \quad nested function class：增加网络层后仍能做原先网络预测
  ...
  ResNext：将resnet卷积层通道平分为并行通道，减少计算量。\\  
...
\textbf{Squeeze-Excite Net}

  每一通道计算权重，取softmax。结果的(1, 1, C)和C个原通道参数按元素相乘。即channel wise的attention操作\\
\textbf{ShuffleNet}

  1.group convolution：使得卷积操作仅对部分channel进行操作。2.交换channel顺序，使得group conv得到其他channel信息
...
\textbf{双向循环神经网络}
  ...
  \textbf{双向 + deep RNN：应stack多个双向RNN组成deep bi-RNN，非每一direction为deep RNN}\\
\textbf{经验总结}
...
\textbf{Transformer}
  ...
  \quad 有可学习参数$\gamma = \vec{1}, \beta = \vec{0}$，形状同一样本特征形状
...
\textbf{BERT}
  ...
  \quad \quad segment embedding：区分两输入的词序，前句segment embedding为全0向量，后句为全1向量。
  ...
  \quad 在小数据集上微调BERT效果不稳定，由于1.原BERT使用的adam删除了bias correction 步骤，实际使用完整BERT。2. 增加微调epoch数
