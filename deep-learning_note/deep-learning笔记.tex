...
% --------------------------------------------------------------------------
\section{自然语言识别}
\noindent \textbf{词嵌入}：将词和实数向量对应，词间夹角$\frac{x^Ty}{||x||\space||y||} \in [-1, 1]$为词的相似度\\
\textbf{跳字模型}：
  ...
  \quad $\mathcal{V} $：所有词的索引集合

  \quad $w_i$：一个词，索引为$i$

  \quad $v_i$：词$w_i$对应的实数向量

  \quad 背景窗口大小$n$：背景词$\in$中心词左$n$ + 右$n$的范围

  \textbf{似然函数}：代表一段词序发生的可能性

  \quad 给定中心词$w_c$，有单个背景词$w_o$的概率$P(w_o | w_c) = \frac{exp(u_o^Tv_c)}{\sum_{i\in\mathcal{V} } exp(u_i^Tv_c)}$

  \quad 对中心词$w_c$，产生所有窗口内的背景词的概率为(假设背景词间independent)$P(c) = \prod_{c-n<j<c+n} P(w_j | w_c)$
...
\textbf{连续词袋模型}
  ...
  同跳字模型有中心词\ 背景词，有$P(w_o | w_c) = \frac{exp(u_o^Tv_c)}{\sum_{i\in\mathcal{V} } exp(u_i^Tv_c)}$不同处：中心词由背景词产生，和跳字模型相反
