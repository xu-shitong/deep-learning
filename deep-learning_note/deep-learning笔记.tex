...
Interest Point Detection 
  ...
  \quad 定义$E(u, v) = \sum_{(x, y) \in W} w(x, y)[h(x + u, y + v) - h(x, y)]^2$
  ...
  \quad \quad $h(x, y)$即(x, y)位置的总体斜率
  
  \quad Taylor expansion：

  \quad \quad $h(x + u, y + v) = h(x, y) + uh_x(x, y) + vh_y(x, y)$

  \quad \quad 带入得$E(u, v) = [u, v]\sum_{(x, y) \in W}w(x, y)
    \begin{bmatrix}
      h_x(x, y)^2 & h_x(x, y)h_y(x, y) \\
      h_x(x, y)h_y(x, y) & h_y(x, y)^2
      \end{bmatrix}
    \begin{bmatrix}
      u \\ 
      v
  ...
  \quad $M = \sum_{(x, y) \in W}w(x, y)\sigma^2
    \begin{bmatrix}
      h_x(x, y, \sigma)^2 & h_x(x, y, \sigma)h_y(x, y, \sigma) \\
      h_x(x, y, \sigma)h_y(x, y, \sigma) & h_y(x, y, \sigma)^2
      \end{bmatrix}$

  \quad \quad **其中$h_x(x, y, \sigma)$为**

  Laplacian of Gaussian LoG：

  \quad 一像素输出值为此像素的二阶导，即$\Delta f = \frac{d^2f}{dx^2} + \frac{d^2f}{dy^2}$
...
Optic Flow
  ...
  由假设得$h(x + u, y+v, t+1) = h(x, y, t) \approx h(x, y, t) + \frac{dh}{dx}u + \frac{dh}{dy}v + \frac{dh}{dt}$

  \quad 则对每一像素(x, y)都有$\frac{dh}{dx}u + \frac{dh}{dy}v + \frac{dh}{dt} = 0$

  Lucas-Kanade Method：计算(u, v)

  \quad 对每一像素$p_i$：$[\frac{dh}{dx}(p_i), \frac{dh}{dy}(p_i)][u, v]^T = [-\frac{dh}{dt}(p_i)]$

  \quad 令
    $A = 
    \begin{bmatrix}
      \frac{dh}{dx}(p_1) & \frac{dh}{dy}(p_1) \\ 
      \frac{dh}{dx}(p_2) & \frac{dh}{dy}(p_2) \\ 
      ...
      \end{bmatrix}$, 
    $x = [u, v]^T$,
    $b = 
  ...
  \quad \quad $A^TA = \sum_{p_i}
    \begin{bmatrix}
      \frac{dh}{dx}(p_i)^2 & \frac{dh}{dx}(p_i)\frac{dh}{dy}(p_i) \\ 
      \frac{dh}{dx}(p_i)\frac{dh}{dy}(p_i) & \frac{dh}{dy}(p_i)^2
      \end{bmatrix}$

  \quad \quad $A^Tb = - \sum_{p_i}
    \begin{bmatrix}
      \frac{dh}{dx}(p_i)\frac{dh}{dt}(p_i) \\ 
      \frac{dh}{dy}(p_i)\frac{dh}{dt}(p_i)
      \end{bmatrix}$

  Horn-Schunck method：每一像素有(u, v)

  \quad 最小化$E = \int_{p_i} (\frac{dh}{dx}(p_i)u + \frac{dh}{dy}(p_i)v + \frac{dh}{dt}(p_i))^2 + \alpha (\Delta u^2 + \Delta v^2) \,dxdy $

  \quad \quad 即最小化临近像素的方向向量改变量

  \quad 由Euler Lagrange equation：

  \quad \quad $(\frac{dh}{dx}(p_i)u + \frac{dh}{dy}(p_i)v + \frac{dh}{dt}(p_i))\frac{dh}{dx}(p_i) - \alpha\Delta u = 0$
  
  \quad \quad $(\frac{dh}{dx}(p_i)u + \frac{dh}{dy}(p_i)v + \frac{dh}{dt}(p_i))\frac{dh}{dy}(p_i) - \alpha\Delta v = 0$
 
  \quad \quad \quad Laplace operator：$\Delta = dxx + dyy$

  \quad 带入$\Delta u = \bar{u} - u$

  \quad \quad $(\frac{dh}{dx}(p_i)^2 + \alpha)u + \frac{dh}{dx}(p_i)\frac{dh}{dy}(p_i)v = \alpha \bar{u} - \frac{dh}{dx}(p_i))\frac{dh}{dt}(p_i)$
  
  \quad \quad $(\frac{dh}{dy}(p_i)^2 + \alpha)v + \frac{dh}{dx}(p_i)\frac{dh}{dy}(p_i)u = \alpha \bar{v} - \frac{dh}{dy}(p_i))\frac{dh}{dt}(p_i)$
