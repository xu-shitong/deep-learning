...
\textbf{deep feedforward network/feedforward neural network/multilayer perceptrons MLP：}
  ...
  \quad 当使用maximum likelihood估计参数时，cost function$J(\theta )$为 训练输入参数的分部 和 训练结果参数的分部 间cross-entropy
  
  \quad $J(\theta ) = -E_{x, y\sim training\_dataset}(log (p_{model}(y | x)))$

  \quad \quad 对于每一在训练集内的(x, y)，求$log (p_{model}(y | x))$， 并求expected value。$p_{model}(y | x)$ 即训练得到的y关于x的分部

  \quad \quad 例：$J(\theta) = -E_{x, y\sim data}(y - f(x;\theta))^2 + const$，当model为$y = N(f(x; \theta), 1)$正则分部时
\end{document}
